function gmm_all = voxceleb_gmm_em(dataList, nmix, final_niter, ds_factor, nworkers, quiet)
...
function [gm, gv] = comp_gm_gv(data)
    ...
    gm = cellfun(@(x) sum(double(x), 2), data, 'UniformOutput', false);
    gm = sum(cell2mat(gm'), 2)/nframes;
    gv = cellfun(@(x) sum(bsxfun(@minus, double(x), gm).^2, 2), data, 'UniformOutput', false);
